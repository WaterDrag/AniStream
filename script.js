...
function simplifyTitle(title) {
    ...
    return title.toLowerCase()
        .replace(/(?:season|série|part|cour)\s*\d+.*$/i, '')
        .replace(/[^a-z0-9\s]/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
}
...
async function fetchAnimeDetail(id) {
    const d = await anilistQuery(`query($id:Int){Media(id:$id,type:ANIME){${FIELDS}}}`, { id });
    return d.Media;
}
...
function openHistory() {
    const h = getHistory(), f = getFavs(), c = document.getElementById('historyContent');
    if (!h.length && !f.length) {
        ...
        const render = (a) => `<div class="anime-card" onclick="closeHistory();openAnimeById(${a.id})"><div class="card-poster"><img src="${a.cover || ''}" loading="lazy"><div class="card-overlay"><div class="play-icon"><svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3"/></svg></div></div></div><div class="card-title">${a.title}</div></div>`;
        let html = '';
...
async function openAnimeById(id) { closeHistory(); openAnime(await fetchAnimeDetail(id)); }
...
{"recommendations":[{"name":"Show Name","genres":"Action, Fantasy","desc":"Short 1 sentence reason why they will like it."}]}`;
    ...
    try {
        ...
        mdTags.forEach(tag => { clean = clean.replace(new RegExp(tag.replace(/`/g, '\\`'), 'g'), ''); });
...
function applyAdvancedFilters() {
    ...
    state.page = 1;
    loadFilterData(state.currentListArgs, false);
}
...
function switchFilter(btn, filterType) {
    ...
    state.currentListArgs = args;
    state.page = 1;
    loadFilterData(args, false);
}
...
async function loadFilterData(args, append = false) {
    ...
    try {
        ...
        if (!append && args.sort === 'TRENDING_DESC' && (!args.genres || args.genres.length === 0) && uniqueItems.length > 0) {
            ...
            tempDiv.innerHTML = heroAnime.description || '';
        ...
        grid.innerHTML = `<div style="color:var(--danger);text-align:center;width:100%">Chyba při načítání dat: ${e.message}</div>`;
    }
}
...
async function selectSearch(id) {
    ...
    openAnime(await fetchAnimeDetail(id));
}
...
async function findSvtSlug(anime, onProgress) {
    ...
    titles.forEach(t => {
        ...
        if (words.length > 1) queries.add(words[0] + ' ' + words[1]);
    });

    const uniqueQueries = Array.from(queries);
    for (const q of uniqueQueries) {
        try {
            if (onProgress) onProgress(`Hledám na SVT: ${q}…`);
            const s = await svtSearch(q);
            if (s) return s;
        } catch (e) { }
    }
    return null;
...
async function openAnime(anime) {
    ...
    addHistory({ id: anime.id, title: getTitle(anime), cover: anime.coverImage?.large || anime.coverImage?.extraLarge });
    showPlayerView();
    destroyHls();
    ...
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = anime.description || 'Bez popisu';
    ...
    try {
        const slug = await findSvtSlug(anime, (msg) => {
            epList.innerHTML = `<li style="padding:20px;text-align:center;color:var(--text-3);">${msg}</li>`;
        });
        if (!slug) throw new Error('NOT_FOUND');
        state.svtSlug = slug;
        ...
        if (!tvShowId) throw new Error('NOT_FOUND');
        state.svtTvShowId = tvShowId;

        const sChecks = await Promise.all(Array.from({ length: 8 }, (_, i) => i + 1).map(async s => {
        ...
        let targetS = 1;
        const mMatch = getTitle(anime).toLowerCase().match(/(?:season|part)\s*(\d+)/);
        if (mMatch) targetS = parseInt(mMatch[1]);
...
function renderSeasonTabs() {
    ...
    tabs.innerHTML = state.availableSeasons.map(s => `<button class="season-tab ${s === state.currentSeason ? 'active' : ''}" onclick="switchSeason(${s})">S${s}</button>`).join('');
}

async function switchSeason(season) {
    if (season === state.currentSeason && state.allSeasons[season]) return;
    state.currentSeason = season;
    document.querySelectorAll('.season-tab').forEach(b => { b.classList.toggle('active', parseInt(b.textContent.replace('S', '')) === season); });
    ...
    try {
        if (!state.allSeasons[season]) {
            ...
            const epMatches = [...epHtml.matchAll(/href="[^"]*\/serial\/[^/]+\/(s\d+e\d+)[^"]*"/g)];
            const nameMatches = [...epHtml.matchAll(/class="ep_name[^"]*"[^>]*>\s*([^<]+)\s*</g)];
            state.allSeasons[season] = epMatches.map((m, i) => {
                const epNum = parseInt(m[1].match(/e(\d+)/)[1]);
                return { number: epNum, title: (nameMatches[i] ? nameMatches[i][1].trim() : null) || `Epizoda ${epNum}`, code: m[1], slug: state.svtSlug, season };
            });
        }
        ...
        document.getElementById('epList').innerHTML = `<li style="color:var(--danger);padding:20px;text-align:center;">Chyba: ${e.message}</li>`;
    }
}
...
function renderEpList() {
    ...
    list.innerHTML = state.episodes.map((ep, i) => {
        ...
        const curr = state.currentEp?.number === ep.number && state.currentEp?.season === ep.season;
...
function setupMainPlayBtn() {
    ...
    if (firstUnwatchedIdx === -1) {
        document.getElementById('mainPlayBtnText').textContent = "Přehrát znovu";
    } else {
        document.getElementById('mainPlayBtnText').textContent = firstUnwatchedIdx > 0 ? `Pokračovat (Ep. ${state.episodes[firstUnwatchedIdx].number})` : "Začít sledovat";
    }
}

function playNextOrFirstEp() {
    let idx = state.episodes.findIndex(ep => !isEpWatched(state.currentAnime.id, ep.number, ep.season));
    playEp(idx === -1 ? 0 : idx);
}
...
async function setFallbackSub(lang) {
    ...
    if (!video) return;
    const btnEn = document.getElementById('fbSubEn');
    const btnCz = document.getElementById('fbSubCz');
    btnEn.classList.remove('active');
    btnCz.classList.remove('active');

    if (lang === 'en') {
        btnEn.classList.add('active');
        switchTrack(video, 'en');
    } else if (lang === 'cs') {
        btnCz.classList.add('active');
        if (state.fallbackCzVttUrl) {
            switchTrack(video, 'cs');
        } else {
            ...
            try {
                ...
                track.label = 'Czech (AI)';
                track.srclang = 'cs';
                track.src = state.fallbackCzVttUrl;
                track.id = 'track-cs';
                video.appendChild(track);
                setTimeout(() => { switchTrack(video, 'cs'); }, 150);
                btnCz.innerHTML = 'AI Překlad CZ (Aktivní)';
            } catch (e) {
                showToast('Chyba AI překladu: ' + e.message, false);
                btnCz.innerHTML = originalText;
                btnEn.classList.add('active');
                btnCz.classList.remove('active');
                switchTrack(video, 'en');
            } finally {
                btnCz.style.pointerEvents = 'auto';
            }
        }
    }
}
...
Enjoy your anime with Gemini AI translations!`;
        ...
        const blob = new Blob([state.fallbackEnVtt], { type: 'text/vtt' });
    ...
    try {
        ...
        loadDiv.innerHTML = `<span style="color:var(--danger)">${e.message}</span>`;
    }
}
...
async function translateSubsWithGemini(vttContent) {
    ...
    txt = txt.replace(/```json|```vtt|```/g, '').trim();
...
function updateNavBtns() {
    document.getElementById('prevEpBtn').disabled = state.currentEpIndex <= 0;
    document.getElementById('nextEpBtn').disabled = state.currentEpIndex >= state.episodes.length - 1;
}
...
function toggleFav() {
    ...
    let f = getFavs();
    const idx = f.findIndex(x => x.id === state.currentAnime.id);
    if (idx >= 0) { f.splice(idx, 1); showToast('Odebráno z oblíbených'); }
    else { f.unshift(state.currentAnime); showToast('Přidáno do oblíbených', true); }
    setFavs(f);
    updateFavBtn();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Spuštění výchozího filtru
    const activeTab = document.querySelector('.filter-tab.active');
    if (activeTab) {
        switchFilter(activeTab, 'TRENDING');
    } else {
        loadFilterData({ sort: 'TRENDING_DESC' }, false);
    }
});
